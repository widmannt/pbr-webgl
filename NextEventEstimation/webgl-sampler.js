...
SamplerProgram.fragShaderSource = `
    ...
    Interaction traceRay(inout Ray r) {
        ...
        intersectObject(SPHERE(vec3(2.0, -2.0, 1.0), 1.0, GLASS(1.5)), r, i);
        ...
        switch (i.material.type) {
            ...
            case M_DIELECTRIC:
                ...
                if (nextRandom() < f) {
                    ...
                    // bsdf = f*i.material.data.rgb/abs(dot(i.outgoing, i.normal))
                    // pdf = f
                    // attenuation *= abs(dot(incoming.direction, i.normal))*bsdf/pdf
                    attenuation *= i.material.data.rgb; // dot(i.outgoing, i.normal) == dot(incoming.direction, i.normal)
                } else {
                    offsetLocation(i.location, -oNormal);
                    incoming = Ray(i.location, refract(i.outgoing, oNormal, eta), MAX_VALUE);
                    // bsdf = (1.0 - f)*eta*eta*i.material.data.rgb/abs(dot(i.outgoing, i.normal))
                    // pdf = 1.0 - f
                    // attenuation *= abs(dot(incoming.direction, i.normal))*bsdf/pdf
    ...
    vec3 sampleGlobalIllumination(vec3 x, vec3 v) {
        ...
        for (uint b = 0u; b <= bounces && beta != vec3(0.0); ++b) {
            ...
            if (NEXT_EVENT_ESTIMATION) {
                ...
                if (i.material.type == M_EMISSIVE) {
                    radiance += i.material.type == M_EMISSIVE ? beta*i.material.data.rgb : vec3(0.0);
                }
            }
    ...
    void main() {
        ...
        NEXT_EVENT_ESTIMATION = screenLocation.x >= 0.0;
